...
const getCleanedContact = async (args: string[], client: Client, WhatsAppBot: WhatsAppBot) => {
    var jidNumber = '';
    ...
    if (parseInt(args[0]) === NaN || args[0][0] === "+" || args[0][0] === "@") {
        if (args[0][0] === "@" || args[0][0] === "+") {
            ...
            client.sendMessage(WhatsAppBot.chatId,"*Enter valid contact number.* Approved Syntax:\n```1. XXXXXXXXXX``` \n```2. Tag the person``` \n```3. +(YYY)XXXXXXXXXX.``` \n_(YY- Country Code, without zeros)_", MessageType.text);
            return undefined;
        }
    } else {
        jidNumber = args[0];       
...
const handleError = async (err, client, WhatsAppBot, customMessage = "```Something went wrong. The error has been logged in log chats```") => {
    console.log(chalk.redBright.bold("[ERROR] " + err));
    let data = {
        commandName: WhatsAppBot.commandName,
        fromMe: WhatsAppBot.fromMe,
        isReply: WhatsAppBot.isReply,
        isGroup: WhatsAppBot.isGroup,
        isPm: WhatsAppBot.isPm,
        isImage: WhatsAppBot.isImage,
        isBotGroupAdmin: WhatsAppBot.isBotGroupAdmin,
        isSenderGroupAdmin: WhatsAppBot.isSenderGroupAdmin,
        isSenderSudo: WhatsAppBot.isSenderSUDO,
        err: err
    }
    client.sendMessage(WhatsAppBot
.chatId, customMessage, MessageType.text);
    client.sendMessage(WhatsAppBot
.logGroup, format(ERROR_TEMPLATE, data), MessageType.text);
}
