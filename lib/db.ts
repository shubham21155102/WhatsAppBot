const data = {
    general: {
        ...
        SUDO_PERMISSION:
            "```Hey there, I am 💠WhatsAppBot💠. I guess you were trying to use my commands in``` *{WhatsAppBot.groupName}*```. However, the bot is currently in {worktype} mode. This enables only the owner and sudo users to use the command``` *{commandName}* ```.\n\nIf you are keen to use my features, you can deploy WhatsAppBot on your own account without much effort, in less than 5 minutes! Check out the links given below.```\n\n🔗https://myWhatsAppBot.com\n🔗https://github.com/shubham21155102/WhatsAppBot",
        ERROR_TEMPLATE:
    ...
    cf: {
        DESCRIPTION: "Codeforces Practices Of an user",
        EXTENDED_DESCRIPTION: "```Get the list of problems solved by a user on Codeforces.```",
        ENTER_USER: "```Enter the username with command```",
        USER_NOT_FOUND: "```User not found.```",
        FETCHING: "```Fetching data...```",

    },
    cfa: {
        DESCRIPTION: "Solution Of all questions of input contest",
        EXTENDED_DESCRIPTION: "```Get the solution of all the problems of the contest.```",
        ENTER_USER: "```Enter the username with command```",
        USER_NOT_FOUND: "```User not found.```",
        FETCHING: "```Fetching Solutions...```",

    },
    cfs: {
        DESCRIPTION: "Solution Of a question Available on codeforces",
        EXTENDED_DESCRIPTION: "```Get the solution of problems of Codeforces.```",
        ENTER_USER: "```Enter the Qurstion Code with command```",
        USER_NOT_FOUND: "```Question not found.```",
        FETCHING: "```Below Is The Solution for your Question...```",
    ...
    getdp: {
        ...
        EXTENDED_DESCRIPTION:
            "```Get the profile picture of the group in a group conversation or the profile picture of WhatsAppBot itself in personal chat. Tag a person in a group to get their profile picture.```",
        IMAGE_CAPTION: "```Here is the display image. Procured by WhatsAppBot.```",
    ...
    help: {
        ...
        HEAD: "🌀 *WhatsAppBot Menu* 🌀\n```Use .help command for detailed info on a module.```",
        TEMPLATE: "\n\n🤖 *Command* - ```{}```\n💡 *Info* - ```{}```",
        COMMAND_INTERFACE: "🌀 *WhatsAppBot Command Interface* 🌀\n\n",
