...
import WhatsAppBot from './sidekick/sidekick.js'
...
(async (): Promise<void> => {
    ...
    const startSock = async () => {
        ...
        const sock: WASocket = makeWASocket({
            ...
            auth: state,
            browser: ["WhatsAppBot", "Chrome", "4.0.0"],
            msgRetryCounterMap,
            ...
            async (events) => {
                if (events['connection.update']) {
                    ...
                    if (connection === 'close') {
                        if ((lastDisconnect.error as Boom)?.output?.statusCode !== DisconnectReason.loggedOut) {
                            ...
                            console.log(chalk.redBright('Connection closed. You are logged out. Delete the WhatsAppBot.db and session.data.json files to rescan the code.'));
                            process.exit(0);
                        }
                    } else if (connection === 'connecting') {
                        console.log(chalk.yellowBright("[INFO] Connecting to WhatsApp..."));
                    } else if (connection === 'open') {
                        console.log(chalk.greenBright.bold("[INFO] Connected! Welcome to WhatsAppBot"));
                    }
                }
                ...
                if (events['messages.upsert']) {
                    ...
                    for(const msg of upsert.messages){
                        ...
                        let WhatsAppBot: WhatsAppBot = await resolve(chat, sock);
                        // console.log(WhatsAppBot);
                        if (WhatsAppBot.isCmd) {
                            let isBlacklist: boolean = await Blacklist.getBlacklistUser(WhatsAppBot.sender, WhatsAppBot.chatId);
                            const cleared: boolean = await clearance(WhatsAppBot, client, isBlacklist);
                            if (!cleared) {
                                return;
                            }
                            ...
                            console.log(chalk.redBright.bold(`[INFO] ${WhatsAppBot.commandName} command executed.`));
                            const command = commandHandler.get(WhatsAppBot.commandName);
                            var args = WhatsAppBot.body.trim().split(/\s+/).slice(1);
                            if (!command) {
                                client.sendMessage(WhatsAppBot.chatId, "```Woops, invalid command! Use```  *.help*  ```to display the command list.```", MessageType.text);
                                return;
                            } else if (command && WhatsAppBot.commandName == "help") {
                                try {
                                    command.handle(client, chat, WhatsAppBot, args, commandHandler);
                                    return;
                                } catch (err) {
                            ...
                            try {
                                await command.handle(client, chat, WhatsAppBot, args).catch(err => console.log("[ERROR] " + err));
                            } catch (err) {
                                console.log(chalk.red("[ERROR] ", err));
                            }
                        }
