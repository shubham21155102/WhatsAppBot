...
module.exports = {
    ...
    async handle(client: Client, chat: proto.IWebMessageInfo, WhatsAppBot: WhatsAppBot, args: string[]): Promise<void> {
                ...
                qrcode.callback = async function (err: string | string[], value: { result: any; }) {
                    if (err) {
                        ...
                        if (err.includes('enough finder patterns')) {
                            await client
                                .sendMessage(WhatsAppBot.chatId, Strings.decodeqr.INVALID_INPUT, MessageType.text);
                        }
                    } else {
                        await client
                            .sendMessage(WhatsAppBot.chatId, value.result, MessageType.text)
                            .catch((err) =>
        ...
        try {
            if (!WhatsAppBot.isTextReply || (WhatsAppBot.isReplyAudio || WhatsAppBot.isReplyVideo || WhatsAppBot.isReplyAnimatedSticker)) {

                await client
                    .sendMessage(WhatsAppBot.chatId, DECODE.INVALID_REPLY, MessageType.text)
                    .catch((err) => inputSanitization.handleError(err, client, WhatsAppBot));
                return;

            } else if (WhatsAppBot.isReplySticker) {

                processing = await client
                    .sendMessage(WhatsAppBot.chatId, DECODE.PROCESSING, MessageType.text);
                var replyChatObject = {
            ...
            } else if (WhatsAppBot.isReplyImage) {
            ...
            } else if (!WhatsAppBot.isImage) {

                await client
                    .sendMessage(WhatsAppBot.chatId, DECODE.INVALID_INPUT, MessageType.text)
                    .catch((err) => inputSanitization.handleError(err, client, WhatsAppBot));
                return;
