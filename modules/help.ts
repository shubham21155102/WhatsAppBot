...
module.exports = {
    ...
    async handle(client: Client, chat: proto.IWebMessageInfo, WhatsAppBot: WhatsAppBot, args: string[], commandHandler: Map<string, Command>): Promise<void> {
        try {
            ...
            if(!args[0]){
                ...
                client.sendMessage(WhatsAppBot.chatId, helpMessage, MessageType.text).catch(err => inputSanitization.handleError(err, client, WhatsAppBot));
                return;
            }
            ...
            if(command){
                ...
                if(command.demo?.isEnabled) {
                    ...
                    await client.sendMessage(WhatsAppBot.chatId, buttonMessage, MessageType.buttonsMessage).catch(err => inputSanitization.handleError(err, client, WhatsAppBot))
                    return;
                }
            ...
            client.sendMessage(WhatsAppBot.chatId, HELP.COMMAND_INTERFACE + "```Invalid Command. Check the correct name from```  *.help*  ```command list.```", MessageType.text).catch(err => inputSanitization.handleError(err, client, WhatsAppBot));
        } catch (err) {
